...
function genUser {
  ...
  echo "generate key and cert for user ${user}@${org} with carrier ${carrier}"
  if [ -d "${WORK}" ]; then
    echo "cleanup ${WORK}"
    rm -R "${WORK}"
  fi
  ...
  ${FAB_PATH}/bin/fabric-ca-client register --caname ${caname} --tls.certfiles ${tlsca} --id.name ''"${user}@${org}"'' --id.secret ${user}pw --id.type client --id.attrs 'carrier='"${carrier}"':ecert,alias='"${user}@${org_name}"',email='"${user}@${org}"''
  export FABRIC_CA_CLIENT_HOME=${WORK}/${user}\@${org}
  ${FAB_PATH}/bin/fabric-ca-client enroll -u https://${user}@${org}:${user}pw@localhost:${port} --caname ${caname} --tls.certfiles ${tlsca} --enrollment.attrs "carrier,alias,email" -M ${FABRIC_CA_CLIENT_HOME}/msp
...
# generate Shipping Global users for cid-based rules
genUser nlsadm NLS org1
genUser slsadm SLS org2
genUser iot service org1
